...
<header>
  <div class="center">
    ...
    <ul class="languages">
      <?php
        if($_SESSION['lang'] == 'es'){
        $output = '<li class="active"><a title="Castellano" href="'.BASE_URL.'es">Español</a></li>
               <li><a title="" href="'.BASE_URL.'ca">Català</a></li>';
        }else{
            $output = '<li><a title="" href="'.BASE_URL.'es">Español</a></li>
                   <li class="active"><a title="'.BASE_URL.'ca" href="">Català</a></li>';
        }
        echo $output;
      ?>
    </ul>
...
<section>
  <div class="center">
    <article class="services">
      <ul>
        <li>
          <?php
            $output = '';
            $hrefs1 = Array ('','trazabilidad','clasificacion','paradas');
            $hrefs2 = Array ('','tracabilitat','clasificacio','parades');
            if($_SESSION['lang'] == 'ca'){
                $href = $hrefs2;
            }else{
                $href = $hrefs1;
            ...
            for($i=1;$i<=3;$i++){
                ...
                $output .= '<h2>'.traducir($seccion['titulo'.$i.'']).'</h2>
                            <figure><img alt="'.traducir($seccion['alt_img'.$i.'']).'" title="'.traducir($seccion['desc_img'.$i.'']).'" src="'.BASE_URL.'userfiles/info/'.$seccion['foto'.$i.''].'" width="305" height="198"></figure>
                            <p>'.traducir($seccion['texto'.$i.'']).'</p>
                            <a title="" href="'.BASE_URL.$_SESSION['lang'].'/'.$href[$i].'">'. lang('Más información').'</a> </li>';
            }
            echo $output;
          ?>
      </ul>
    ...
    <article class="about">
        <?php
            $output = '<h3>'.traducir($seccion['titulo4']).'</h3>
                        <figure><img title="'.traducir($seccion['desc_img4']).'" src="'.BASE_URL.'userfiles/noticia/'.$seccion['foto4'].'" alt="'.traducir($seccion['alt_img4']).'" width="100" height="130"></figure>
                        <p>'.traducir($seccion['texto4']).'</p>';
            echo $output;
    ...
    <article class="gallery">
        <?php
            ...
            $output = '<h3>'.traducir($galeria['nombre']).'</h3>
                        <ul>';
